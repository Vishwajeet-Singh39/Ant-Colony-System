...
#calculating the visibility of the next node visibility(i,j)=1/d(i,j)
...
#intializing the rute of the ants with size rute(n_ants,n_nodes+1) 
#note adding 1 because we want to come back to the source node

rute = np.ones((m,n+1))

for ite in range(iteration):
    
    rute[:,0] = 1          #initial starting and ending positon of every ants '1' i.e node '1'
    ...
    for i in range(m):
        ...
        for j in range(n-1):
            ...
            node = np.nonzero(cum_prob>r)[0][0]+1       #finding the next node having probability higher then random(r) 

            
            rute[i,j+1] = node              #adding node to route 
           
        left = list(set([i for i in range(1,n+1)])-set(rute[i,:-2]))[0]     #finding the last untraversed node to route
        
        rute[i,-2] = left                   #adding untraversed node to route
